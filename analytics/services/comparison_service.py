...
from analytics.services.draft_service import DraftContext, get_draft_contexts_for_players, get_draft_contexts_for_player
...
def parse_player_search_filters(params) -> PlayerSearchFilters:
    """Normalize GET parameters for staff player search."""
...
def _draft_status_for_contexts(contexts: list[DraftContext]) -> str:
    if not contexts:
...
def _apply_draft_status_filter(players: list[Player], draft_status: str) -> list[Player]:
    ...
    return [
        ...
        if _draft_status_for_contexts(contexts_by_player.get(player.id, [])) == wanted
    ]
...
def search_players(filters: PlayerSearchFilters) -> PlayerSearchResult:
    """Search canonical players for the staff-facing Phase 6 player experience."""
